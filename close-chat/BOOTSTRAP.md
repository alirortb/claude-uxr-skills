...
**Performance context (paste-in if relevant):** If this session's work counts toward current performance objectives — or toward durable competencies a future review cycle will read — paste that context inline in the "session covered" block at the bottom of this message. Claude won't have it otherwise, and step 8 below depends on it. Objectives may be unsettled (a prior plan closed, the successor set not yet agreed); say so in the paste and step 8 will tag competencies only. If nothing applies, skip the paste and step 8 will say "skipped — no performance context provided." (This block intentionally stays generic so the paste-block file is safe to keep in a public repo.)
...
8. **Performance tracking** — if performance context was pasted in the session-covered block: for each deliverable/decision this session produced, tag which current objective it counts toward (primary/secondary tagging is fine when work counts toward two) **and** which durable competencies it evidences. If the objective set is unsettled — a prior plan closed and its successor isn't agreed yet — say so and tag competencies only; never score against a closed plan or report coverage percentages against it. Note days remaining to the next review touchpoint the user named, and flag in words any objective with no evidence this cycle. **Only if** the session earned an objective tag, a review touchpoint is ~2 weeks out or nearer, or the user flagged an outstanding record gap: note whether the periodic on-track/off-track judgment and the objectives' own records (due date + status) exist in writing — a judgment that lives only in conversation is an open thread. Otherwise skip this check silently rather than repeating it every session. If a stakeholder meeting occurred: note whether actions/decisions were captured + the right people informed. If a chapter/course/development-area commitment is in scope: log progress (or explicit lack of it). If earlier sessions were tagged against a since-closed plan, leave those tags as written — the scheme change is forward-only, so don't retroactively re-tag or restate them. If no performance context was pasted: say "skipped — no performance context provided" by name.
...
Performance context (if any): <paste current objectives, review cadence + next touchpoint, competency tags, OR write "none" / "objectives not yet agreed">
Which objective(s) / competencies this session counts toward: <name them from the paste, or "none">
```
