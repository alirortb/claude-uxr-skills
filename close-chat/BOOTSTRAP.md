...
**The eight-item closeout checklist** — for each item, either produce the draft text or explicitly say "skipped — <reason>". Silence is not a pass.
...
8. **Next move** — one concrete sentence describing the first thing Anthony should do when he picks up next session. Specific names, paths, numbers — not vague "continue the work." If no obvious next move, say so honestly: "Next move: none — this session was self-contained."

**Output format**: a single message structured as an 8-item bulleted recap (one line each, "step name — what was updated or skipped"), followed by the actual draft text for items that produced artifacts, each clearly headed so Anthony can copy each block straight into the right local file. End with the **Next move** line set apart visually so it stands out as the handoff into the next session.
