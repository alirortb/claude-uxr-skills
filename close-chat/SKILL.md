...
- **Competency tag (durable)** — the deliverables log is career-evidence that outlives the turnaround plan (feeds company perf reviews too — see its `README.md` → "Tiers & promotion cadence"). So also apply the durable **competency tags** from `~/dev/eod-recaps/taxonomy.local.md` (C-Impact/Scope/Craft/Influence/Ownership — currently a placeholder pending Miro's real leveling rubric) to any Deliverables-Log entry this session writes, so it's review-ready, not just checkpoint-ready. The W-tags and competency tags are independent dimensions.
